...
class PostHelpers {
	...
	static function formatContent( $content )
	{
	    ...
	    $content = preg_replace_callback('#\<php\>(.+?)\<\/php\>#s',function(
		    $matches
		  ){
        $attr["code"] = $matches[1];
		    return  view("core.code", $attr);
        }, $content);
	    $content = preg_replace_callback('#\<pre\>(.+?)\<\/pre\>#s',function(
		    $matches){return "<pre class=\"prettyprint lang-php\">".htmlentities($matches[1])."</pre>";}, $content);
